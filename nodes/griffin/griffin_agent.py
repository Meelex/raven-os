...
# ── Monster name pools by floor tier ─────────────────────────────────────────
MONSTERS = [
    ["Sand Scorpion", "Tomb Rat",       "Desert Jackal",  "Dusty Asp"],
    ["Bone Servant",  "Carrion Crow",   "Dune Hyena",     "Pale Scarab"],
    ["Grave Warden",  "Croc Shaman",    "Desert Revenant","Sand Golem"],
    ["Crypt Stalker", "Whirlwind Djinn","Asp Cultist",    "Iron Scarab"],
    ["Tomb Colossus", "Desert Lich",    "Shadow Pharaoh", "Void Asp"],
]

def _monster_name(floor):
    tier = min((floor - 1) // 2, len(MONSTERS) - 1)
...
STATE_FILE = "/tmp/scarab_pet_state.json"

def _push_event(text, etype):
    """Append event to log, keep last 5."""
    log = state.get("event_log", [])
    log.append({"text": text, "type": etype})
    state["event_log"] = log[-5:]

def _write_state(last_event="", event_type="idle"):
    try:
        import json as _j
...
def _fight():
    ...
    hp     = state["hp"]
    mon_name = mon["name"]
    _push_event("vs " + mon_name, "combat")
    _write_state("vs " + mon_name, "combat")
    ...
    if hp <= 0:
        # Death — respawn at full HP
        deaths = state.get("consecutive_deaths", 0) + 1
        state["consecutive_deaths"] = deaths
        if deaths >= 2 and state["floor"] > 1:
            old_floor = state["floor"]
            state["floor"] = max(1, state["floor"] - 1)
            state["consecutive_deaths"] = 0
            _log(f"[RETREAT] {deaths} deaths — retreating F{old_floor} -> F{state['floor']}")
        else:
            _log(f"[DEATH] Floor {floor} (deaths: {deaths}) — respawning at full HP")
        state["hp"] = state["max_hp"]
        return False, 0, 0, "", mon_name
    ...
    return True, xp_gained, gold_gained, loot_name, mon_name
...
def main():
    ...
    while True:
        try:
            ...
            # Run one fight
            companion_mode = state["companion_mode"]
            won, xpg, gg, loot, mon_name = _fight()
            ...
            if won:
                ...
                if loot:
                    ...
                    ev_text = "Slew " + mon_name
                    ev_type = "win"
                _push_event(ev_text, ev_type)
                _write_state(ev_text, ev_type)
                ...
                _push_event("Fell to " + mon_name, "death")
                _write_state("Fell to " + mon_name, "death")
                _sync()
