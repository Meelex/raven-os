...
epaper_driver_display::epaper_driver_display(int width, int height,custom_lcd_spi_t _lcd_spi_data) : 
    ...
    buffer = (uint8_t *)heap_caps_malloc(lcd_spi_data.buffer_len, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
	assert(buffer);
}
...
void epaper_driver_display::spi_gpio_init() {
	...
	gpio_conf.pull_up_en = GPIO_PULLUP_DISABLE;   // BUSY is actively driven; pull-up causes false HIGH
	ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_config(&gpio_conf));
...
void epaper_driver_display::read_busy() {
    ...
    int level = gpio_get_level((gpio_num_t)busy);
    printf("[EPD] read_busy: initial level=%d\n", level);
    fflush(stdout);
    while(gpio_get_level((gpio_num_t)busy) == 1)
	{
        vTaskDelay(pdMS_TO_TICKS(5));   //LOW: idle, HIGH: busy
        if (esp_timer_get_time() - start > 5000000LL) {  // 5s timeout
            printf("[EPD] read_busy: TIMEOUT (BUSY stuck HIGH on GPIO %d) - continuing\n", busy);
            fflush(stdout);
            break;
        }
    }
    printf("[EPD] read_busy: done after %lld ms\n", (esp_timer_get_time() - start) / 1000);
    fflush(stdout);
}
...
void epaper_driver_display::EPD_Init() {
    printf("[EPD] EPD_Init: hardware reset\n"); fflush(stdout);
    set_rst_1();
    ...
    printf("[EPD] EPD_Init: wait busy after reset\n"); fflush(stdout);
    read_busy();
    printf("[EPD] EPD_Init: SWRESET\n"); fflush(stdout);
    EPD_SendCommand(0x12);  //SWRESET
    printf("[EPD] EPD_Init: wait busy after SWRESET\n"); fflush(stdout);
    read_busy();
    printf("[EPD] EPD_Init: configuring panel\n"); fflush(stdout);
...
void epaper_driver_display::EPD_Display() {
    int buffer_len = lcd_spi_data.buffer_len;
    assert(buffer);
    // SSD1681 full refresh LUT compares old frame (0x26) vs new frame (0x24).
    // Write all-white to old frame so every pixel transition is treated as
    // "from white" — gives the LUT correct reference for the first display.
    static uint8_t white_frame[5000];
    memset(white_frame, 0xFF, buffer_len);
    EPD_SendCommand(0x26);
    writeBytes(white_frame, buffer_len);
    // Write new frame
